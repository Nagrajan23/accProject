...
aRaw0 = zeros(10,3,'double');
for i = 1:10
    aRaw0(i,:) = [cosd(60+(i-1)*10),sind(60+(i-1)*10),0];
end

avRaw0 = zeros(10,3,'double');
avRaw0(:,3) = 10;
...
%     aRaw(:,i) = filtfilt(b1 ,a1 , aRaw1(:,i));
    ...
    avRaw(:,i) = avRaw(:,i) - mean(avRaw(1:1,i));
...
%         magNoG(:,i) = wden(aMotion(:,i),'modwtsqtwolog','s','mln',8,'sym4');
        magNoG(:,i) = magNoG(:,i) - mean(magNoG(1:1,i));
        
        thresh = max(abs(magNoG(1:1,i)));
